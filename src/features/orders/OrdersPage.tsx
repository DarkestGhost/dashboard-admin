...
import type { Order, OrderStatus, Column } from "@/types/feature";
...
const columns: Column<Order>[] = [
  ...
  { key: "name", header: "مشتری" },
  {
    key: "totalPrice",
    header: "مبلغ",
    render: (row: Order) => formatPrice(row.totalPrice),
  },
  {
    key: "status",
    header: "وضعیت",
    render: (row: Order) => {
      ...
      return (
        <span
          className={`px-2 py-1 rounded-full text-sm transition-colors duration-300 ease-linear ${status?.color || "bg-gray-500"}`}
        >
          {status?.name || "نامشخص"}
        </span>
      );
  ...
  },
  {
    key: "createdAt",
    header: "تاریخ",
    render: (row: Order) => new Date(row.createdAt).toLocaleDateString("fa-IR"),
  },
];
...
const OrdersPage = () => {
  ...
  const { mutate } = useMutation({
    ...
    onMutate: async (data: Partial<Order> & { id: number }) => {
  ...
  const handleConfirmDialog = () => {
    ...
    mutate({ id: selectedOrder?.id, status: newStatus as OrderStatus });
    handleCloseDialog();
  };
  ...
  return (
    <div className="p-4 sm:p-6">
      ...
      <ConfirmDialog
        title={"اطلاعات محصولات سفارش"}
        isOpen={isOpenDialog}
        onClose={handleCloseDialog}
        onConfirm={user.role === "admin" ? handleConfirmDialog : undefined}
        confirmText={user.role === "admin" ? "ذخیره" : undefined}
        cancelText="باشه"
      >
        {selectedOrder && (
          <div className="text-base font-vazir_regular flex flex-col justify-center gap-y-4">
            ...
            <p className="flex items-center gap-x-2">
              ...
              {user.role === "admin" ? (
                ...
                statusOrderOptions.find((s) => s.slug === selectedOrder.status)
                  .name
              )}
            </p>
