...
import type { Product, Category, Column } from "@/types/feature";
...
const ProductsPage = () => {
  ...
  const categoryMap = useMemo(() => {
    ...
    return categorySearchOptions.reduce(
  ...
  const columns: Column<Product>[] = useMemo(
    () => [
      { key: "name", header: "نام محصول" },
      {
        key: "category",
        header: "دسته‌بندی",
        render: (row: Product) => categoryMap[row.category] || row.category,
      },
      {
        key: "price",
        header: "قیمت",
      ...
      },
      {
        ...
        render: (row: Product) => (
          <span
            className={`px-2 py-1 rounded-full text-xs transition-colors duration-300 ease-linear ${
              row.stock > 0
  ...
  if (categoryIsError)
    return (
      <ErrorMessage message={categoryError.message} onRetry={categoryRefetch} />
    );
