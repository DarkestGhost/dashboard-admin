...
const DashboardPage = () => {
  ...
  const lowStockChartProducts = products.filter(
    (product) => product.stock > 0 && product.stock <= 5,
  ).length;

  const inventoryValueByCategory = products?.reduce(
    (acc: Record<string, number>, product) => {
      acc[product.category] =
        (acc[product.category] || 0) + product.price * product.stock;
      return acc;
    },
    {},
  );
    ...
    <div className="p-4 sm:p-6">
      ...
      <div className="mt-12 grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <InventoryStatusChart
          ...
          lowStockProducts={lowStockChartProducts}
        />
      </div>
