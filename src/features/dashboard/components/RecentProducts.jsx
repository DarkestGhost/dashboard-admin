        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5 shadow-sm transition-all duration-300 ease-linear">
            <div className="flex items-center justify-between mb-5">
                ...
                <span className="text-xs text-zinc-500 font-vazir_regular">
                    {`${formatNumber(5)} محصول اخیر`}
                </span>
            </div>
            {recentProducts.map(product => (
                <div key={product.id} className="flex items-center justify-between py-5 border-b last:border-0 border-zinc-200 dark:border-zinc-800 transition-colors duration-300 ease-linear">
                    ...
                    <span className={`px-2 py-1 rounded-full text-xs font-vazir_medium ${product.stock === 0 ? "bg-red-100 text-red-600" : "bg-green-100 text-green-600"}`}>
                        {product.stock === 0 ? "ناموجود" : `${formatNumber(product.stock)} عدد`}
                    </span>
                </div>
            ))}
        </div>
